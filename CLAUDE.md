...
- **`internal/tips`** — six rule-based tips (`cache-hit-low`, `high-output-ratio`, `many-sessions`, `low-read-edit-ratio`, `unused-mcp-servers`, `long-session`). `AllTips` calls `OverviewTotals`, `ToolCallCounts`, and `LongestRecentSession`, then filters against dismissed tip keys. Tips define a static `Body` or, when the message needs runtime values (e.g. session id + turn count), an optional `BodyFn(stats) string` that overrides it. The MCP-configured count is cached for 30 s via `mcpConfiguredCache`; tests that swap `ConfiguredMCPLoader` must call `tips.ResetMCPCache()` to invalidate it.
- **`app/app.go`** — `App` struct with all exported methods Wails binds to `window.go.App.*()`. `Startup` launches `scanLoop` (30 s ticker, emits `"scan"` Wails event after changes). `GetContextHealth()` reads `~/.claude/settings.json` (MCP server count, hook count, file size) and `~/.claude/CLAUDE.md` (line count, bullet-rule count, file size) — it takes no range parameters and is intentionally excluded from the range-reactive `fetchAll()` in `OverviewView.vue`; it loads once on mount and only refreshes when the user clicks the Refresh button.
...
The `brew-tap` job writes to `Casks/tokentally.rb` (not `Formula/` — this is a macOS `.app` cask, and Homebrew only resolves `brew install --cask` against a tap's `Casks/` directory). It runs `mkdir -p Casks` before writing, so no placeholder file needs to pre-exist in `payfacto/homebrew-tap`. The job runs after all platform matrix builds complete and only fires on `v*` tag pushes.
